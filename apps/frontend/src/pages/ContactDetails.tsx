...
import { Button, Card, CardContent, Chip, Link, MenuItem, Stack, TextField, Typography } from '@mui/material';
...
export function ContactDetails() {
  ...
  const load = async (targetPage = page) => {
    ...
    const [cRes, aRes] = await Promise.all([
      api.contactsShow(token, contactId),
      api.contactsActivitiesIndex(token, contactId, { per_page: 10, page: targetPage, kind: kind || undefined, happened_after: after || undefined, happened_before: before || undefined }),
    ]);
    if (cRes.ok && cRes.data) setContact((cRes.data as any).contact);
  ...
  useEffect(() => { load(); }, [token, contactId]);

  const addNote = async () => {
    if (!token || !contactId || !note.trim()) return;
    const res = await api.contactsActivitiesCreate(token, contactId, { kind: 'note', content: note });
  ...
  return (
    <Stack spacing={2}>
      <Stack direction="row" spacing={1} alignItems="center">
        ...
        <Typography variant="h5" fontWeight={800}>Contact: {[contact.first_name, contact.last_name].filter(Boolean).join(' ') || contact.email}</Typography>
      </Stack>
        ...
        <Card sx={{ flex: 1 }} className="glass">
          <CardContent>
            ...
            <Stack direction="row" spacing={1} flexWrap="wrap">
              {contact.email && <Chip label={contact.email} />}
              {contact.phone && <Chip label={contact.phone} />}
              {contact.title && <Chip label={contact.title} />}            
              {contact.company?.name && (
                <Link component={RouterLink} to={`/${lng || 'en'}/contacts`} underline="hover">{contact.company.name}</Link>
              )}
            </Stack>
